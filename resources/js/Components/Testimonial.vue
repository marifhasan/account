<template>
    <form @submit.prevent="submitDoc" class="relative">
        <div class="border border-gray-300 rounded-lg shadow-sm overflow-hidden focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500">
            <label for="personality" class="sr-only">Personality</label>
            <input type="text" v-model="form.personality" class="block w-full border-0 pt-2.5 text-lg font-medium placeholder-gray-500 focus:ring-0" placeholder="Personality" />

            <label for="testimonial" class="sr-only">Testimonial</label>
            <textarea rows="2" v-model="form.description" class="block w-full border-0 py-0 resize-none placeholder-gray-500 focus:ring-0 sm:text-sm" placeholder="Write a testimonial..." />

            <!-- Spacer element to match the height of the toolbar -->
            <div aria-hidden="true">
                <div class="py-2"> <div class="h-9" /> </div> <div class="h-px" />
                <div class="py-2"> <div class="py-px"> <div class="h-9" /> </div> </div>
            </div>
        </div>
                                ...
                                <ListboxOption as="template" v-for="assignee in assignees" :key="assignee.value" :value="assignee" v-slot="{ active }">
                                    <li :class="[ active ? 'bg-gray-100' : 'bg-white', 'cursor-default select-none relative py-2 px-3', ]">
                                        <div class="flex items-center">
                                            ...
                                            <span class="ml-3 block font-medium truncate"> {{ assignee.name }} </span>
                                        </div>
                                    </li>
                                </ListboxOption>
                            </ListboxOptions>
            ...
            <div class="border-t border-gray-200 px-2 py-2 flex justify-between items-center space-x-3 sm:px-3">
                <div class="flex">
                    <button type="button" class="-ml-2 -my-2 rounded-full px-3 py-2 inline-flex items-center text-left text-gray-400 group">
                        <RefreshIcon @click=" form.id = form.personality = form.description = null" class="-ml-1 h-5 w-5 mr-2 group-hover:text-gray-500" aria-hidden="true" />
...
export default {
    ...
    setup(props) {
        ...
        const form = reactive({
            ...
            personality: props.testimonial?.personality,
            description: props.testimonial?.description,
        })

        function submitDoc() {
            Inertia.patch(route('testimonial.update'), form)
            form.id = null;
