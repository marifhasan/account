<template>
    <Menu as="div" class="ml-3 relative inline-block text-left">
        <div>
            <MenuButton class="p-2 rounded-full flex items-center text-gray-400 hover:text-gray-600">
                ...
                <div class="py-1">
                    <MenuItem>
                        <Link :href="detailHref?? '#'" :class="[!detailHref ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2 text-sm hover:bg-gray-100']">
                            <ExternalLinkIcon :class="['mr-3 h-5 w-5 text-gray-400', !detailHref? '' : 'group-hover:text-gray-500']" aria-hidden="true" />
                            Detail
                        </Link>
                    </MenuItem>
                    <MenuItem>
                        <Link :href="editHref?? '#'" :class="[!editHref ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2 text-sm hover:bg-gray-100']">
                            <PencilAltIcon :class="['mr-3 h-5 w-5 text-gray-400', !editHref? '' : 'group-hover:text-gray-500']" aria-hidden="true" />
                            Edit
                        </Link>
                    </MenuItem>
                </div>
    ...
    methods: {
        deleteRecord() {
            if(confirm(this.deleteMessage)) {
                Inertia.delete(this.deleteHref);
            }
        }
    },
