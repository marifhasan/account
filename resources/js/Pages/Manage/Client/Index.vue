            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex justify-between px-4 py-5 border-b border-gray-200 sm:px-6">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium">Clients</p>
                    ...
                    <table class="table-fixed min-w-full w-full" id="table">
                        <thead>
                            <tr>
                                ...
                                <th class="px-5 py-3 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Contact</th>
                                <th class="px-5 py-3 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white">
                            <tr v-for="(client, index) in clients" :key="client.id" :class="[index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b']">
                                ...
                                <td class="px-5 py-2 whitespace-no-wrap">
                                    <div class="flex items-center">
                                        <div class="ml-4">
                                            <div class="text-sm leading-5 font-semibold text-gray-700">{{ client.name }}</div>
                                            <div class="text-sm leading-5 text-gray-600">{{ client.address }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-5 py-2 whitespace-no-wrap text-sm leading-5 text-gray-600">
                                    <div class="flex items-center">
                                        <div class="ml-4">
                                            <div class="text-sm leading-5 font-semibold text-gray-700">{{ client.contact_name }}</div>
                                            <div class="text-sm leading-5 text-gray-600">{{ client.contact_number }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-5 py-2 whitespace-no-wrap">
                                    <div class="flex justify-end">
                                        <Link :href="route('client.show', client.id)" class="text-primary-600 hover:text-primary-800 ml-3" title="detail">
                                            <ExternalLinkIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <Link :href="route('client.edit', client.id)" class="text-indigo-600 hover:text-indigo-800 ml-3" title="edit">
                                            <PencilAltIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <button @click="deleteRecord(client.id)" class="text-rose-600 hover:text-rose-800 ml-3" title="delete">
    ...
    methods: {
        deleteRecord(record_id) {
            if(confirm("Do you really want to delete this client?")) {
                Inertia.delete(route('client.destroy', record_id));
            }
        }
    },
