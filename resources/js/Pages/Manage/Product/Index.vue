            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex justify-between px-4 py-5 border-b border-gray-200 sm:px-6">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium"> Products </p>
                    ...
                    <table class="table-fixed min-w-full w-full" id="table">
                        <thead>
                            <tr>
                                ...
                                <th class="px-5 py-3 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left"> Company </th>
                        ...
                        <tbody class="bg-white">
                            <tr v-for="(product, index) in products" :key="product.id" :class="[ index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b', ]">
                                <td class="px-5 py-2 whitespace-no-wrap text-sm leading-5 text-gray-700 text-center"> {{ index + 1 }} </td>
                                <td class="px-5 py-2 whitespace-no-wrap text-sm leading-5 text-gray-700">
                                    <div class="flex justify-between">
                                        {{ product.name }}
                                        <a :href="product.href" target="_blank" class="text-gray-600 hover:text-gray-800 ml-3" title="product link">
                                ...
                                <td class="px-5 py-2 whitespace-no-wrap text-sm leading-5 text-gray-700">
                                    <div class="flex justify-between">
                                        ...
                                        <a :href="product.company_href" target="_blank" class="text-gray-600 hover:text-gray-800 ml-3" title="company link">
                                            <ExternalLinkIcon class="w-6 h-6" aria-hidden="true" />
                                        </a>
                                    </div>
                                </td>
                                <td class="px-5 py-2 whitespace-no-wrap">
                                    <div class="flex justify-end">
                                        <Link :href=" route( 'product.show', product.id ) " class="text-primary-600 hover:text-primary-800 ml-3" title="detail">
                                            <ExternalLinkIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        ...
                                        <button @click="deleteRecord(product.id)" class="text-rose-600 hover:text-rose-800 ml-3" title="delete">
    ...
    methods: {
        deleteRecord(record_id) {
            if (confirm("Do you really want to delete this product?")) {
                Inertia.delete(route("product.destroy", record_id));
            }
        },
    },
