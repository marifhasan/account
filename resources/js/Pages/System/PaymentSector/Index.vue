            ...
            <div class="px-4 sm:px-6 lg:max-w-6xl lg:mx-auto lg:px-8">
                <div class="py-6 md:flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
                    ...
                    <div class="mt-6 h-9 flex space-x-3 md:mt-0 md:ml-4">
                        <Link :href="route('sector.create')" class="inline-flex items-center w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white hover:bg-gray-50 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-primary-400">
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex justify-between px-4 py-5 border-b border-gray-200 sm:px-6">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium">Sectors (Payment)</p>
                    ...
                    <table class="table-fixed min-w-full w-full" id="table">
                        ...
                        <tbody class="bg-white">
                            <tr v-for="(sector, index) in sectors" :key="sector.id" :class="[index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b']">
                                ...
                                <td class="px-1 py-2 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 text-gray-600">
                                    <div class="text-sm leading-5 text-gray-700">{{ sector.name }}</div>
                                </td>
                                <td class="px-5 py-2 whitespace-no-wrap">
                                    <div class="flex justify-end">
                                        <Link :href="route('sector.edit', sector.id)" class="text-indigo-600 hover:text-indigo-800 ml-3" title="edit">
                                            <PencilAltIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <button @click="deleteRecord(sector.id)" class="text-rose-600 hover:text-rose-800 ml-3" title="delete">
...
export default {
    ...
    methods: {
        deleteRecord(record_id) {
            if(confirm("Do you really want to delete this sector?")) {
                Inertia.delete(route('sector.destroy', record_id));
            }
        }
    },
    ...
    setup() {
        const breadcrumbs = [
            { name: 'Sectors (Payment)', href: route('sector.index'), current: false },
