            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex justify-between px-4 py-5 border-b border-gray-200 sm:px-6">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium">Jobs</p>
                    ...
                    <table class="table-fixed min-w-full w-full" id="table">
                        <thead>
                            <tr>
                                ...
                                <th class="px-5 py-3 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Client</th>
                        ...
                        <tbody class="bg-white">
                            <tr v-for="(job, index) in jobs" :key="job.id" :class="[index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b']">
                                <td class="px-5 py-2 whitespace-no-wrap">
                                    <div class="text-sm leading-5 text-gray-700 text-center">{{ (index + 1) }}</div>
                                </td>
                                <td class="px-5 py-2 whitespace-no-wrap">
                                    <div class="text-sm leading-5 text-gray-700">{{ job.client_name }}</div>
                                </td>
                                <td class="px-5 py-2 whitespace-no-wrap">
                                    <div class="flex">
                                        <div class="ml-4">
                                            <div class="text-sm leading-5 font-semibold text-gray-700">{{ job.name }}</div>
                                            <div class="text-sm leading-5 text-gray-600">{{ job.amount }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-5 py-2 whitespace-no-wrap">
                                    <Status :status="job.status" color="green" />
                                </td>
                                <td class="px-5 py-2 whitespace-no-wrap">
                                    <div class="flex justify-end">
                                        <Link :href="route('job.show', job.id)" class="text-primary-600 hover:text-primary-800 ml-3" title="detail">
                                            <ExternalLinkIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <Link :href="route('job.edit', job.id)" class="text-indigo-600 hover:text-indigo-800 ml-3" title="edit">
                                            <PencilAltIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <button @click="deleteRecord(job.id)" class="text-rose-600 hover:text-rose-800 ml-3" title="delete">
    ...
    methods: {
        deleteRecord(record_id) {
            if(confirm("Do you really want to delete this Job?")) {
                Inertia.delete(route('job.destroy', record_id));
            }
        }
    },
